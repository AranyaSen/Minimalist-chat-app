...
router.post('/messages/receiver/', async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ message: 'Failed to fetch messages', err: err });
    }
});
...
router.post('/messages/sender', async (req, res) => {
    ...
    try {
        ...
        return res.status(201).json({ message: 'Message sent successfully', content: newMessage });
    } catch (error) {
        return res.status(500).json({ message: 'Failed to send message', error: error });
    }
})
